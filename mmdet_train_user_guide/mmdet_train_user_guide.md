...
LabelImg标注的xml文件转json文件：参考代码：[xml2coco.py]()，参考链接：https://blog.csdn.net/lcqin111/article/details/103146945
...
（1）在这个目录下新建一个py文件，例如：`underwater_dataset.py`，然后把这个目录下的`coco.py`的内容复制过来，修改class类名为`Underwater_Dataset`，最后把`CLASSES`的那个tuple改为自己数据集对应的种类tuple即可，例如：

```bash
CLASSES = ('holothurian','echinus','scallop','starfish')  #数据集标签类别
```

（2）在`mmdet\datasets\__init__.py`文件中引入自定义的数据集。在这个py文件中开头加入：

```bash
from .underwater_dataset import  Underwater_Dataset
```

（3）在`mmdet\datasets\__init__.py`文件的`__all__`列表中加入自己的`Underwater_Dataset`。

###   2.加入数据集类别

修改的位置：`mmdet\core\evaluation\`

（1）在`mmdet\core\evaluation\class_names.py`文件中加入：
...
（2）在`mmdet\core\evaluation\class_names.py`文件中的`dataset_aliases`字典中加入：

```bash
'underwater':['underwater']
```

（3）在`mmdet\core\evaluation\__init__.py`文件中引入自定义的数据集：

在`__all__`列表中加入自己的`underwater_classes`；在开头的`from .class_names import()`中加入 `underwater_classes`。

###   3.引入数据路径

修改的位置：`config\`

（1）在这个目录下创建自己的文件夹，例如`underwater_detection`，用于存储训练自己数据集的不同模型的配置文件。例如训练Faster RCNN：
...
（2）在`config\_base_\models`文件夹下创建自己的模型配置文件，如：`faster_rcnn_r50_fpn_uw.py`，将`config\_base_\models\faster_rcnn_r50_fpn.py`的内容复制过来，修改：

```bash
num_classes=4  #标签类别数量
```

（3）在`config\_base_\datasets`文件夹下创建自己的数据集配置文件`underwater_detection.py`，将`config\_base_\datasets\coco_detection.py`的内容复制过来，修改：
...
根据work_dirs文件夹下的.json文件，分析mAP指标，找到最佳pth文件。例如：在`epoch_12.pth`中，mAP最高。
